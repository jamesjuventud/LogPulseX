# LogPulseX
A small, header-only C++20 logging library: async, lock-free on the hot path, with console, size-based rotating file, daily rotating file, syslog, Hex dumping, file compression, and TCP network sinks, structured (JSON) logging, and crash-safe flushing.
